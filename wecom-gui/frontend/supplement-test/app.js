...
function customer() {
  return customerEl.value.trim() || "补剂测试客户";
}
...
async function refresh({silent = false} = {}) {
  const payload = await api(`/api/supplement-test/status?customer=${encodeURIComponent(customer())}`);
  render(payload);
...
async function send() {
  ...
  try {
    const payload = await api("/api/supplement-test/send", {
      method: "POST",
...
async function reset() {
  ...
  setStatus("正在重置...");
  try {
    const payload = await api("/api/supplement-test/reset", {
      method: "POST",
